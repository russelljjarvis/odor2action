...
def plotly_sized2(K):
    ...
    for edge in K.edges(data=True):
        ...
        y.extend([y0, y1, None])
    edge_trace = Scatter(
        x=x, y=y, line=Line(width=1.5, color="#888"), hoverinfo="none", mode="lines"
    )
    ...
    node_trace = Scatter(
        ...
        marker=Marker(
            ...
            color=group,
            size=[],
            colorbar=dict(title="Class", ticks="outside"),
            line=dict(width=1),
        ),
    )
    xx = []
    ...
    for node in K.nodes():
        ...
        node_info = "# of connections: %s <br />AuthorID: %s" % (
            str(node_deg[node]),
            str(name),
        )
...
def plotly_sized(g):
    ...
    d = nx.degree(g)
    # d = [((d[node]+1) * 1.25) for node in g.nodes()]
    nx.set_node_attributes(g, "size", d)
...
def author_to_sirg_only_network(
    auth_name: str = "", sirg_author_list: List = []
) -> networkx.DiGraph():
    response = requests.get("https://dissem.in/api/search/?authors=" + str(auth_name))
    author_papers = response.json()
    coauthors = []
